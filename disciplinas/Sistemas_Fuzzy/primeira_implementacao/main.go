...
func main() {
	...
	print_grau_pertinencia_funcao_triangular(funcao_triagular_escolhida, altura)

	aux_value := funcao_triagular_escolhida[0]
	for {
		print_grau_pertinencia_funcao_triangular(funcao_triagular_escolhida, aux_value)
		aux_value = aux_value + step
		if aux_value > funcao_triagular_escolhida[2] {
			break;
		}
	}
	...
	print_grau_pertinencia_funcao_trapezoidal(funcao_trapezoidal_escolhida, altura)

	aux_value = funcao_trapezoidal_escolhida[0]
	for {
		print_grau_pertinencia_funcao_trapezoidal(funcao_trapezoidal_escolhida, aux_value)
		aux_value = aux_value + step
		if aux_value > funcao_trapezoidal_escolhida[3] {
			break;
		}
	}
...
func print_grau_pertinencia_funcao_triangular(funcao_triagular [3]float64, altura float64) {
	if altura <= funcao_triagular[0] {
		fmt.Printf("Grau de pertinencia = 0\n")
	} else if altura > funcao_triagular[0] && altura <= funcao_triagular[1] {
		fmt.Printf("Grau de pertinencia = %f\n", (altura - funcao_triagular[0])/(funcao_triagular[1] - funcao_triagular[0]))
	} else if altura > funcao_triagular[1] && altura <= funcao_triagular[2] {
		fmt.Printf("Grau de pertinencia = %f\n", (funcao_triagular[2] - altura)/(funcao_triagular[2] - funcao_triagular[1]))
	} else if altura > funcao_triagular[2] {
		fmt.Printf("Grau de pertinencia = 0\n")
	}
}

func print_grau_pertinencia_funcao_trapezoidal(funcao_trapezoidal [4]float64, altura float64) {
	if altura <= funcao_trapezoidal[0] {
		fmt.Printf("Grau de pertinencia = 0\n")
	} else if altura > funcao_trapezoidal[0] && altura <= funcao_trapezoidal[1] {
		fmt.Printf("Grau de pertinencia = %f\n", (altura - funcao_trapezoidal[0])/(funcao_trapezoidal[1] - funcao_trapezoidal[0]))
	} else if altura > funcao_trapezoidal[1] && altura <= funcao_trapezoidal[2] {
